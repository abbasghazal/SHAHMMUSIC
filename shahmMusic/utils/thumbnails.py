...
async def gen_thumb(videoid, user_id, theme):
    ...
    try:
        ...
        try:
            text_w, text_h = draw.textsize(f"shahm MUSIC IS PLAYING OP", font=font)
            draw.text(
                ((1280 - text_w) / 2, 30),
                f"shahm MUSIC IS PLAYING OP",
                fill="red",
                font=font,
            )
            text_w, text_h = draw.textsize(
                f"shahm Music One Of The Most Advanced Telegram Music Bot", font=arial
            )
            draw.text(
                ((1280 - text_w) / 2, 80),
                f"shahm Music One Of The Most Advanced Telegram Music Bot",
                fill="green",
                font=arial,
            )
            if para[0]:
...
async def gen_qthumb(videoid, user_id, theme):
    ...
    try:
        ...
        para = textwrap.wrap(title, width=32)
        try:
            text_w, text_h = draw.textsize(f"shahm ADDED THIS SONG TO QUEUE", font=font)
            draw.text(
                ((1280 - text_w) / 2, 30),
                f"shahm ADDED THIS SONG TO QUEUE",
                fill="red",
                font=font,
            )
            text_w, text_h = draw.textsize(
